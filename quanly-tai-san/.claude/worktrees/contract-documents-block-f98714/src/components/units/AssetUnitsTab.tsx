...
export function AssetUnitsTab({ assetId }: { assetId: string }) {
  ...
  const update = useMutation({
    mutationFn: ({ id, body }: { id: string; body: UnitInput }) =>
      assetsApi.units.update(assetId, id, body),
    onSuccess: () => {
        ...
        <Card>
          <CardContent className="py-12 text-center">
            ...
            <p className="text-xs text-muted-foreground mt-1">
              Thêm phòng nếu bạn cho thuê lẻ từng phần.
            </p>
              ...
              <CardContent className="p-4 flex items-center justify-between gap-3 flex-wrap">
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <Badge variant="outline" className={UNIT_STATUS_CLASS[u.status]}>
                    ...
                    {u.area != null && (
                      <span className="text-xs text-muted-foreground">{u.area} m²</span>
                    )}
                  </div>
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {removeErr ? (
                <span className="text-destructive">{removeErr}</span>
              ) : (
                <>
                  Phòng <b>{removeTarget?.name}</b> sẽ bị xoá.
                </>
              )}
            </AlertDialogDescription>
          </AlertDialogHeader>
...
function UnitFormDialog({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>{initial ? "Sửa phòng/tầng" : "Thêm phòng/tầng"}</DialogTitle>
        </DialogHeader>
        <div className="space-y-3">
          ...
          {initial && (
            <div className="space-y-1.5">
              ...
              <Select
                value={String(status)}
                onValueChange={(v) => setStatus(Number(v) as UnitStatusCode)}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  {enumOptions(UNIT_STATUS).map((o) => (

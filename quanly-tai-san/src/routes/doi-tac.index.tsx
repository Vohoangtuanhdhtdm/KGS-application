  ...
  return (
    <div className={embedded ? "space-y-5" : "p-6 space-y-5 max-w-[1400px]"}>
      <div className="flex items-end justify-between flex-wrap gap-3">
        <div>
          {!embedded && <h1 className="text-2xl font-semibold tracking-tight">Sổ đối tác</h1>}
          <p className="text-sm text-muted-foreground mt-1">
            {data ? `${data.totalCount} liên hệ` : "Đang tải..."} · Người thuê, chủ nhà, môi giới,
            nhà thầu.
          </p>
        </div>
        <Button
      ...
      <Card>
        <CardContent className="p-4 grid grid-cols-1 md:grid-cols-2 gap-3">
          ...
          <Select
            ...
            onValueChange={(v) => {
              setType(v === "all" ? "" : (Number(v) as ContactTypeCode));
              setPage(1);
            }}
          >
            <SelectTrigger>
        ...
        <CardContent className="p-0">
          {query.isLoading ? (
            ...
            <div className="p-16 text-center">
              ...
              <p className="text-sm text-muted-foreground mt-1">
                Thêm người thuê/chủ nhà để tạo hợp đồng.
              </p>
              <Button
                className="mt-5"
                onClick={() => {
              ...
              <TableBody>
                {data!.items.map((c) => (
                  <TableRow key={c.id}>
                    ...
                    <TableCell>
                      <ContactTypeBadgeCode code={c.type} />
                    </TableCell>
                    <TableCell>
                      {c.phone ? (
                        <a
                          href={`tel:${c.phone}`}
                          className="text-sm flex items-center gap-1.5 hover:text-primary"
                        >
        ...
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              onClick={(e) => {
                e.preventDefault();
                if (deleting) removeMut.mutate(deleting.id);
              }}

  ...
  const value: Store = {
    ...
    addContract: (c, autoReminders = true) => {
      ...
      if (c.unitId && c.direction === "Cho thuê") {
        setUnits((prev) =>
          prev.map((u) => (u.id === c.unitId ? { ...u, status: "Đang cho thuê" } : u)),
        );
      }
      if (autoReminders) {
        const rentTitle =
          c.direction === "Cho thuê" ? `Thu tiền thuê HĐ ${c.code}` : `Đóng tiền thuê HĐ ${c.code}`;
    ...
    renewContract: (oldId, nc) => {
      setContracts((prev) => [
        nc,
        ...prev.map((c) => (c.id === oldId ? { ...c, status: "Đã gia hạn" as const } : c)),
      ]);
    },
      ...
      setContracts((prev) =>
        prev.map((c) =>
          c.id === id
            ? {
                ...c,
                status: "Đã chấm dứt" as const,
                endDate: terminationDate,
                notes: `[Chấm dứt] ${reason}${c.notes ? " — " + c.notes : ""}`,
              }
            : c,
        ),
      );
